...
await sql`CREATE INDEX IF NOT EXISTS roleta_spins_lead_id_idx ON roleta_spins (lead_id);`;
await sql`CREATE INDEX IF NOT EXISTS roleta_spins_created_at_idx ON roleta_spins (created_at DESC);`;
...
await sql`INSERT INTO roleta_settings (id) VALUES (1) ON CONFLICT (id) DO NOTHING;`;
...
await sql`
  ...
  VALUES
    ...
    ('20% off matrícula + 20% off 1ª mensalidade Holística', 'desconto-holistica', 8, NULL, TRUE, 4, '#009CDE', 'Apresente este comprovante na Holística para resgatar o desconto.', 12, 0),
    ('Instalação + 1 mês grátis de internet', 'internet-gratis', 8, NULL, TRUE, 5, '#FFC20E', 'Fale com a equipe Antonov para ativar sua promoção.', 12, 0),
    ('1 mês grátis na academia', 'academia-gratis', 8, NULL, TRUE, 6, '#009CDE', 'Apresente este comprovante na academia parceira.', 12, 0)
  ON CONFLICT (slug) DO NOTHING;
`;

console.log('Tabelas roleta (premios, leads, spins, settings) prontas.');
